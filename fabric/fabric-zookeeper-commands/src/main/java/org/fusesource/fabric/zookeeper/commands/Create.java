...
public class Create extends ZooKeeperCommandSupport {
    ...
    protected void doExecute(CuratorFramework curator) throws Exception {
        ...
        try {
            if (recursive) {
                curator.create().creatingParentsIfNeeded().withMode(mode).withACL(acls).forPath(path, nodeData.getBytes("UTF-8"));
            } else {
                curator.create().withMode(mode).withACL(acls).forPath(path, nodeData.getBytes("UTF-8"));
            }
        } catch (KeeperException.NodeExistsException e) {
            if (overwrite) {
                curator.setData().forPath(path, nodeData.getBytes("UTF-8"));
            } else {
                throw e;
            }
        }
