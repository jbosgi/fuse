...
import org.fusesource.fabric.groups.{Group, ChangeListener, Singleton, NodeState}

/**
 * @author <a href="http://hiramchirino.com">Hiram Chirino</a>
 */
trait BaseNodeState extends NodeState {
...
class TextNodeState extends BaseNodeState {
  @BeanProperty
  @JsonProperty
  var id:String = _
}
...
object ClusteredSupport {
  ...
  def encode(value: AnyRef, out: OutputStream, mapper: ObjectMapper) {
...
class ClusteredSingletonWatcher[T <: NodeState](val stateClass:Class[T]) extends ChangeListenerSupport {
  ...
  val LOG: Logger = LoggerFactory.getLogger(classOf[ClusteredSingletonWatcher[T]])

  protected var _group:BaseGroup = _
  def group = _group
  ...
  private val listener = new ChangeListener() {
    def changed() {
      ...
      val t = new util.LinkedHashMap[String, T]()
      members.foreach {
        case (path, data) =>
          try {
  ...
  def start(group:Group) {
    this.synchronized {
      ...
      _group = group.asInstanceOf[BaseGroup]
      _group.add(listener)
    }
  }
  ...
  def stop() {
    this.synchronized {
      ...
      _members = mutable.HashMap[String, ListBuffer[(String, T)]]()
      _group = null
    }
  }
  ...
  protected var _members = mutable.HashMap[String, ListBuffer[(String,  T)]]()
  def membersMap = this.synchronized { _members }

  def changed_decoded(m: util.LinkedHashMap[String, T]) {
    this.synchronized {
      if( _group!=null ) {
        _members = mutable.HashMap[String, ListBuffer[(String,  T)]]()
        m.foreach { case node =>
...
class ClusteredSingleton[T <: NodeState](stateClass:Class[T]) extends ClusteredSingletonWatcher[T](stateClass)
  ...
  def join(state:T) {
    this.synchronized {
      ...
      this._state = state
      _eid = _group.join(encode(state, mapper))
    }
  }
  ...
  def master : T = {
    this.synchronized {
      if(this._state==null)
        throw new IllegalStateException("Not joined")
      val t = _members.get(this._state.id).map(_.head._2)
      if (t.isEmpty)
        null.asInstanceOf[T]
      else
        t.get
  ...
  def slaves : util.List[T] = this.synchronized {
    if(this._state==null)
      throw new IllegalStateException("Not joined")
  ...
  def members : util.Map[String, T] = this.synchronized {
    _members.mapValues(_.head._2)
  }
